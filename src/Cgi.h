...
public:
    ...
    char const * getFile() const throw() {
        return _file;
    }
    int getLine() const throw() {
        return _line;
    }
    char const * getFunction() const throw() {
        return _func;
    }
    HttpResponseCode const & getResponseCode() const throw() {
        return _code;
    }
...
public:
    ...
    /// Returns the name and version of the gateway interface (usually @c CGI/1.1).
    std::string const & getGatewayInterface() const {
        return _gatewayInterface;
    }
    /// Returns the name and version of the protocol (usually @c HTTP/1.0 or @c HTTP/1.1).
    std::string const & getServerProtocol() const {
    ...
    std::string const & getRequestMethod() const {
    ...
    std::string const & getPathTranslated() const {
    ...
    /// Returns the query string for the request; usually only valid for a @c GET.
    ...
    std::string const & getCookieString() const {
    ...
    size_t getNumValues(std::string const &key) const {
