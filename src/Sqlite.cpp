...
{
    _status = sqlite3_prepare_v2(
        db._db, stmt.c_str(), static_cast<int>(stmt.size()), &_stmt, NULL);
    if (_status != SQLITE_OK) {
        throw HTTP_EXCEPT(HttpResponseCode::INTERNAL_SERVER_ERROR, format(
            "Failed to prepare statement: %s", stmt.c_str()));
    }
}
...
pair<string, bool> const SqliteStmt::getString(int i) {
    ...
    char const *val = reinterpret_cast<char const *>(
