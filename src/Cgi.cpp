...
HttpResponseCode const
    HttpResponseCode::NOT_FOUND (404, "Not Found",
...
Environment::Environment (int argc, char const *const *argv)
    : _contentLength (getEnv<size_t> ("CONTENT_LENGTH", 0)),
      ...
      _cookieString (getEnv ("HTTP_COOKIE")), _kvMap (), _cookieMap ()
{
  if (_contentType.empty ()
  ...
  else if (_contentType.compare (0, 19, "multipart/form-data") == 0)
    {
      if (_contentLength == 0)
        ...
        {
          throw HTTP_EXCEPT (
              HttpResponseCode::REQUEST_ENTITY_TOO_LARGE,
  ...
  while (i != string::npos)
    {
      ...
      _kvMap.insert (
          std::make_pair (data.substr (nb, ne - nb), data.substr (h, j - h)));
      i = j + sep.size ();
...
MemoryWriter::MemoryWriter ()
    : _contentLength (0), _cap (1024 * 1024), _content (0)
{
