    ...
    template <typename T> T getEnv(char const *name, T def) {
        ...
        if (value) {
            try {
                ...
                throw HTTP_EXCEPT(HttpResponseCode::BAD_REQUEST, format(
                    "%s could not be converted to a %s", name, typeid(T).name()));
            }
        } else {
            return def;
        }
...
HttpResponseCode const HttpResponseCode::OK(
...
HttpResponseCode const HttpResponseCode::PRECONDITION_FAILED
    (412, "Precondition Failed", "The precondition given in one or more of "
...
HttpResponseCode const HttpResponseCode::REQUEST_ENTITY_TOO_LARGE
    (413, "Request Entity Too Large", "The server is refusing to process a "
...
HttpResponseCode const HttpResponseCode::REQUEST_URI_TOO_LONG
    (414, "Request-URI Too Long", "The server is refusing to service the "
...
void HttpException::writeErrorResponse(ostream & stream) const {
    ...
    ostringstream oss;
    oss << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" \"http://www.w3.org/TR/html4/strict.dtd\">\n"
           "<html>\n"
           "<head><title>" << c << " " << _code.getSummary() << "</title></head>\n"
           "<body>\n"
    ...
    if (c != 401 && c != 403 && c != 404) {
        oss << "<br /><br />\n"
               "<tt>" << getTypeName() << "</tt> thrown at <tt>" << _file << ": " << _line <<
...
void writeErrorResponse(std::ostream & stream, std::exception const &e) {
    ...
    oss << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" \"http://www.w3.org/TR/html4/strict.dtd\">\n"
           "<html>\n"
           "<head><title>" << c.getCode() << " " << c.getSummary() << "</title></head>\n"
           "<body>\n"
...
/** Returns a vector of all cookie (name, value) pairs sent along with the request.
  */
...
void Environment::parsePostInput(string const &data) {
    ...
    while (i != string::npos) {
        ...
        _kvMap.insert(std::make_pair(data.substr(nb, ne - nb),
                                     data.substr(h, j - h)));
        i = j + sep.size();
...
void Environment::parseCookies(string const &data) {
    for (string::size_type i = 0, j = 0; j != string::npos; i = j + 1) {
        ...
        if (sep != string::npos && sep < j) {
            ...
            if (i != sep) {
                if (j == string::npos) {
                     _cookieMap.insert(std::make_pair(
                         data.substr(i, sep - i), data.substr(sep + 1)));
                } else {
                     _cookieMap.insert(std::make_pair(
                         data.substr(i, sep - i), data.substr(sep + 1, j - sep - 1)));
                }
            }
        } 
    }
