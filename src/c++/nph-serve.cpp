...
// Perform basic sanity checking of the CGI environment, including common query parameters.
...
bool parseBool(Environment const &env,
               string const &key,
               bool defaultValue)
{
    ...
    throw HTTP_EXCEPT(HttpResponseCode::BAD_REQUEST, format(
        "Value of %s parameter must equal (case insensitively) one of"
        "1,y[es],t[rue],on or 0,n[o],f[alse],off", key.c_str()));
    return false;
}
...
vector<string> const getDirEntries(
    fs::path const &diskpath,
    string const &dbpath,
    Access const &access
) {
    string sql;
    switch (access.getPolicy()) {
        ...
        case Access::ROW_ONLY:
            sql = "SELECT DISTINCT h.file_name, h.is_dir FROM\n"
                  ...
                  "WHERE\n"
                  "    p.path = '" + dbpath + "' AND\n"
                  "    g.group_id IN (" + groupString(access) + ")";
            break;
        case Access::ROW_DATE:
            sql = "SELECT DISTINCT h.file_name, h.is_dir FROM\n"
                  ...
                  "    p.path = '" + dbpath + "' AND\n"
                  "    (\n"
                  "        g.group_id IN (" + groupString(access) + ") OR\n"
                  "        h.min_pub_date < CURRENT_TIMESTAMP\n"
                  "    )";
            break;
        default:
...
// Strip the three leading path elements (<schema_group>/<schema>/table/) from path,
...
//
// Note that path has the form <schema_group>/<schema>/<table>/<f_1>/<f_2>/.../<f_i>.
...
string const getDirListing(
    ...
    oss << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" \"http://www.w3.org/TR/html4/strict.dtd\">\n"
           ...
           "<ul>\n"
           "<li><a href=\"" << parent.string() << "/\">Parent Directory</a></li>\n";
