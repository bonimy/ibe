...
// requested cutout doesn't overlap the image, \c false is returned. Results are 
// stored in \c box as follows: box[0] = xmin, box[1] = ymin, box[2] = xmax,
// box[3] = ymax.
bool cutoutPixelBox(
    Coords center,     // Cutout center.
    ...
    long *box)         // Pixel-space cutout box; must point to an array of at least 4 longs.
{
...
Coords const parseCoords(Environment const &env,
                         string const &key,
                         Units defaultUnits,
                         bool requirePair)
{
    ...
    if (*s != 0) {
        ...
        if (boost::regex_match(s, _pixRe)) {
            ...
            throw HTTP_EXCEPT(HttpResponseCode::BAD_REQUEST, format(
                "Value of %s parameter has invalid trailing unit "
                "specification", key.c_str()));
        }
    }
    return coords;
}
...
Wcs::Wcs(char * hdr, int nkeys) : _wcs(0), _nwcs(0) {
    int nreject = 0;
    if (::wcspih(hdr, nkeys, WCSHDR_all,
                 0, &nreject, &_nwcs, &_wcs) != 0) {
        throw HTTP_EXCEPT(HttpResponseCode::INTERNAL_SERVER_ERROR,
...
void streamSubimage(boost::filesystem::path const &path,
                    Coords const &center,
                    Coords const &size,
                    Writer &writer)
{
